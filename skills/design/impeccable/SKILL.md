...
license: Apache 2.0
allowed-tools:
  - Bash(sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" *)
  - Bash(sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" *)
---
...
1. Run `sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" context` once per session, where `<skill-base-dir>` is the directory that contains this SKILL.md (the skill folder, not a plugin root two levels above it); keep cwd at the user's project. That base directory resolves every `sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" <verb>` command in this skill and its references, and `$HOME/AppData/Local/hermes/skills/design/impeccable/scripts` is the fallback only when the runtime reports no base directory. On a Windows shell without `sh`, call `sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable"` instead. The launcher runs a self-contained binary that ships next to it or is downloaded once on first run; no Node or other runtime is required. Pass a named source file or route as `--target <path>`. It loads PRODUCT.md, DESIGN.md, the matching surface brief, and native-platform guidance when applicable; follow its directives and do not rerun it.
...
**Pin / Unpin:** `sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" pin <pin|unpin> <command>` creates or removes a standalone `/<command>` shortcut. Report the script's result concisely; relay stderr verbatim on error.
