...
Manual `sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" detect` scans use the same project filter config by default: `detector.ignoreRules`, `detector.ignoreFiles`, `detector.ignoreValues`, and `detector.designSystem.enabled`. `hook.enabled` only controls automatic hook execution, not manual CLI scans. Use `sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" detect --no-config ...` for a raw detector run that ignores project config/context. Use `sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" ignores ...` for direct CLI CRUD on the same detector ignores.
...
2. Invoke the admin script and pass the user's output through verbatim:
   ...
   ```bash
   sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" hooks <action> [args...]
   ```
...
- If the finding has no value-specific command, such as `side-tab`, scope that one rule to the file: `ignore-value <id> "*" --file <path>`. Run `sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" detect <path>` first to see what actually fires there.
...
```bash
sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" hooks ignore-value overused-font Inter --shared --reason "User confirmed Inter is intentional"
```

Example self-served exception, with the evidence named:

```bash
sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" hooks ignore-value bounce-easing bounce-ball --shared --reason "Agent: literal ball-bounce animation, bounce easing is the subject"
```

Example whole-rule font exception:

```bash
sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" hooks ignore-rule overused-font --all-values --reason "User asked to ignore overused fonts generally"
```
...
```bash
sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" hooks ignore-value design-system-font-size "*" --file "src/overlay/widget.js" --reason "Injected widget builds its own type scale; DESIGN.md's ramp describes the site"
```

Example whole-file exception, for a file that is out of scope entirely:

```bash
sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" hooks ignore-file "src/legacy/Card.tsx"
```
