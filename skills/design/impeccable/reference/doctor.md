...
- **Tool version.** The installed skill is older than the published one. `impeccable context` reports that at boot as `UPDATE_AVAILABLE` and `sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" update` fixes it. Not this command's job.
...
```
sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" doctor --json
```
...
- **`auto`** carries no decision. Run `sh "$HOME/AppData/Local/hermes/skills/design/impeccable/scripts/impeccable" doctor --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
